...
if (!array_key_exists('ajax', $_GET)) {
    ...
    if ($_GET['ajax'] == "account_activation") {
        ...
        if ($_SESSION['activation_key'] == "") {
            header("Location: ../error.php");
            exit(); // Ensures that the script stops running after the redirect
        }
        $_SESSION['kld_email'] = "";
        $try = mysqli_query($conn, "Select * from admin_acc where admin_activation_key = '" . $_SESSION['activation_key'] . "'");
        while ($row = $try->fetch_array()) {
            echo "";
            $_SESSION['kld_email'] = $row['admin_email'];
            $_SESSION['kld_role'] = $row['admin_role'];
                            ...
                            <div class="login-signin">
                                ...
                                <form class="form text-center" id="kt_login_signup_form">
                                    ...
                                    <div class="form-group ">
                                        <input
                                            class="form-control h-auto text-white bg-white-o-5 rounded-pill border-0 py-4 px-8"
                                            type="text" placeholder="KLD Role" disabled value="<?php echo $_SESSION['kld_role'] ?>" />
                                    </div>
