function hideOrganizer(id) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      var dataString = "ajax=hide_org" + "&org_acc_id=" + id;
      $.ajax({
        type: "POST",
        url: "ajax.php",
        data: dataString,
...
$("#update_org_submit").click(async function () {
  if (validateForm()) {
    $(this).text("Submitting");
    $(this).prop("disabled", "disabled");
    var profilePic = await imagefileinsert(
      document.getElementById("update_org_profilepic")
    );
    var dataString =
      "ajax=update_org_user" +
      "&update_org_profilepic=" +
      profilePic +
      "&update_org_fname=" +
      $("#update_org_fname").val() +
      "&update_org_lname=" +
      $("#update_org_lname").val() +
      "&update_org_organization=" +
      $("#update_org_organization").val() +
      "&update_org_role=" +
      $("#update_org_role").val() +
      "&update_org_kldid=" +
      $("#update_org_kldid").val() +
      "&update_org_email=" +
      $("#update_org_email").val();
    ...
    $.ajax({
      ...
      success: function (html) {
        switch (html) {
          case "success":
            Swal.fire(
              "Organizer account successfully created!",
              "Redirecting to homepage...",
              "success"
            );
            setTimeout(
              function () {
                window.open("index.php?page=organizer", "_self");
              },
              15000 // 5 seconds
            );
            break;
...
$("#add_org_submit").click(async function () {
  if (validateForm()) {
    ...
    $(this).prop("disabled", "disabled");
    var profilePic = await imagefileinsert(
      document.getElementById("add_org_profilepic")
    );
    var dataString =
      "ajax=add_org_user" +
      "&add_org_profilepic=" +
...
function validateForm() {
  ...
  if ($("#update_org_fname").val() == "") {
    Swal.fire("Please specify the first name!", "Please try again!", "error");
    return false;
  } else if ($("#update_org_lname").val() == "") {
    Swal.fire("Please specify the last name!", "Please try again!", "error");
    return false;
  } else if ($("#update_org_organization").val() == "") {
    Swal.fire("Please specify the organization!", "Please try again!", "error");
    return false;
  } else if ($("#update_org_role").val() == "") {
    Swal.fire("Please specify the role!", "Please try again!", "error");
    return false;
  } else if ($("#update_org_kldid").val() == "") {
    Swal.fire("Please specify the KLD Number!", "Please try again!", "error");
    return false;
  } else if ($("#update_org_email").val() == "") {
    Swal.fire("Please specify the email!", "Please try again!", "error");
    return false;
  }
  return true;
