...
<div class="d-flex flex-column-fluid">
	...
	<div class=" container ">
						...
						<div class="flex-grow-1 p-8 card-rounded bgi-no-repeat d-flex"
							...
							<div class="row">
								...
								<div class="col-12 col-md-9">
									...
									<form method="post">
										<a href="javascript:history.back()" class="btn btn-light-primary font-weight-bold py-2">Back</a>
		...
		if (!isset($eventId)) {
			die('Event ID is not defined.');
		}

		// Prepare and execute the query to fetch question categories
		...
		?>
			...
			<!-- Category Title -->
			<div class="alert alert-custom alert-white alert-shadow fade show gutter-b justify-content-center" role="alert">
				<h4 class="text text-center"><?php echo $question_cat; ?></h4>
			</div>
			...
			<div class="row">
				...
				while ($rowQuestion = $tryQuestions->fetch_assoc()) {
					$question_id = $rowQuestion['question_id'];
					$question = $rowQuestion['question'];

					// Fetch response data for each question
					$responseQuery = "
                ...
                FROM response_tbl 
                WHERE question_id = '$question_id' AND event_id = '$eventId'";

					$responseResult = mysqli_query($conn, $responseQuery);
					$responseData = mysqli_fetch_assoc($responseResult);
				?>
					<div class="col-lg-4">
						<div class="card card-custom gutter-b">
							<div class="card-header">
								<div class="card-title">
									<h3 class="card-label"><?php echo $question; ?> </h3>
								</div>
							</div>
							<div class="card-body">
								<!-- Store the response data in data-* attributes -->
								<div id="f_chart_<?php echo $question_id; ?>"
									class="d-flex justify-content-center"
									data-strongly-agree="<?php echo $responseData['strongly_agree']; ?>"
									data-agree="<?php echo $responseData['agree']; ?>"
									data-neutral="<?php echo $responseData['neutral']; ?>"
									data-disagree="<?php echo $responseData['disagree']; ?>"
									data-strongly-disagree="<?php echo $responseData['strongly_disagree']; ?>"></div>
							</div>
						</div>
						<!--end::Card-->
					</div>
