    ...
    <div class=" container ">
        ...
        $try = mysqli_query(
            ...
            "SELECT DISTINCT
             ...
             WHERE (kld_event.status = 'archived' OR kld_event.status = 'cancelled') AND (stakeholder_tbl.status = 'approved' OR stakeholder_tbl.status = 'pending') AND stakeholder_tbl.admin_id = $adminId
        ...
        while ($row = $try->fetch_array()) {
         ...
         LEFT JOIN org_tbl ON kld_event.event_org_id = org_tbl.org_id
         LEFT JOIN category_tbl ON kld_event.category_id = category_tbl.category_id
         WHERE kld_event.status = 'cancelled' AND stakeholder_tbl.status = 'approved' AND stakeholder_tbl.admin_id = $adminId
         ORDER BY `kld_event`.`event_start_date` DESC"
            );
