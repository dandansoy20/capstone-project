    ...
    <div class=" container ">
        ...
        $try = mysqli_query(
            ...
            "SELECT 
             ...
             WHERE kld_event.status = 'completed' AND stakeholder_tbl.status = 'pending' AND stakeholder_tbl.admin_id = $adminId
        ...
        while ($row = $try->fetch_array()) {
                             ...
                             <div class="flex-grow-1 p-12 card-rounded bgi-no-repeat d-flex flex-column justify-content-center align-items-start"
                                  ...
                                  <div class="row">
                                     <div class="col-9">
                                         ...
                                         <a href="?page=completed-view&event_id=' . $row["event_id"] . '">
            ...
            ?><?php
                echo '
                         ...
                         <a href="?page=completed-view&event_id=' . $row["event_id"] . '" class="btn btn-primary font-weight-bold py-2 px-6">View Event</a>
                         </div>
                         <div class="col-3">
         ...
         LEFT JOIN org_tbl ON kld_event.event_org_id = org_tbl.org_id
         LEFT JOIN category_tbl ON kld_event.category_id = category_tbl.category_id
         WHERE kld_event.status = 'completed' AND stakeholder_tbl.status = 'approved' AND stakeholder_tbl.admin_id = $adminId
         ORDER BY `kld_event`.`event_start_date` DESC"
            );
         ...
         LEFT JOIN org_tbl ON kld_event.event_org_id = org_tbl.org_id
         LEFT JOIN category_tbl ON kld_event.category_id = category_tbl.category_id
         WHERE kld_event.status = 'completed' AND stakeholder_tbl.status = 'rejected' AND stakeholder_tbl.admin_id = $adminId
         ORDER BY `kld_event`.`event_start_date` DESC"
                );
