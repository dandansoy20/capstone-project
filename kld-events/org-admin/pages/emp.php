				...
				<div class="card-header border-0 py-5">
					...
					<div class="card-toolbar">
						<div class="btn-group pr-5">
							<button type="button" class="btn btn-light-info dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Organization</button>
							<div class="dropdown-menu">
								<?php
								include('./control/db.php');
								$try = mysqli_query($conn, "Select * from org_tbl");
								while ($row = $try->fetch_array()) {
									echo '<a class="dropdown-item">' . $row['org_name'] . '</a>';
								}
								?>
							</div>
						</div>
						<a href="?page=add-employee" class="btn btn-primary font-weight-bolder font-size-sm">
						...
						<table class="table table-head-custom table-vertical-center" id="kt_advance_table_widget_1">
							...
							<tbody>
								...
								while ($row = $try->fetch_array()) {
									...
									echo '<td class="pl-0">
									<label class="checkbox checkbox-lg checkbox-inline">
										<input type="checkbox" value="' . $row['emp_id'] . '" />
										<span></span>
									</label>
								</td>';
									...
									<div class="symbol symbol-50 symbol-light mt-1">
										<span class="symbol-label">
											<img src="' . $row['emp_profilepic'] . '" class="h-75 align-self-end" alt="" onerror="this.onerror=null; this.src="assets/default.jpg" />
										</span>
									</div>
								</td>
								...
								';
									...
									<a href="?page=overview-emp&id=' . $row['emp_id'] . '" class="btn btn-icon btn-light btn-hover-primary btn-sm">
									...
									<a href="?page=edit-emp&id=' . $row['emp_id'] . '" class="btn btn-icon btn-light btn-hover-primary btn-sm mx-3">
										<span class="svg-icon svg-icon-md svg-icon-primary">
