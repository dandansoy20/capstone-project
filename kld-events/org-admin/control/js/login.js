$("#kld_org_login_button").click(function () {
  var dataString =
    "ajax=logging_in" +
    "&login_type=" +
    $("#login_type").val() +
...
$("#kld_form_submit").click(function () {
  if (
    passwordValidator(
      $("#kld_username_pass1").val(),
      $("#kld_username_pass2").val()
    )
  ) {
    var dataString =
      "ajax=create_account" +
      ...
      $("#login_type").val() +
      "&firstname=" +
      $("#kld_firstname_admin").val() +
      "&lastname=" +
      $("#kld_lastname_admin").val() +
      "&username=" +
      $("#kld_username_admin").val() +
      "&password=" +
      $("#kld_username_pass1").val();
    console.log(dataString);
    $.ajax({
...
function passwordValidator(pass, pass2) {
  if (pass.toString().length < 8) {
    ...
    Swal.fire(
      "Password must contains lowercase character",
      "Please try again!",
      "error"
    );
    return false;
